...
async function loadCounters() {
  ...
  const { count: flowerCount, error: flowerError } =
    await db
      .from("flower_scans")
      .select("*", {
        count: "exact",
  ...
  const { count: joyCount, error: joyError } =
    await db
      .from("Messages")
      .select("*", {
        count: "exact",
...
async function loadMessages() {
  ...
  if (error) {
    ...
    messages.innerHTML =
      "<p>We're taking a little moment. The Joy Garden will be back soon. 🌱</p>";
